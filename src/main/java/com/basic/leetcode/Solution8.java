package com.basic.leetcode;

/**
 * 请你来实现一个 atoi 函数，使其能将字符串转换成整数。
 * <p>
 * 首先，该函数会根据需要丢弃无用的开头空格字符，直到寻找到第一个非空格的字符为止。
 * <p>
 * 当我们寻找到的第一个非空字符为正或者负号时，则将该符号与之后面尽可能多的连续数字组合起来，作为该整数的正负号；假如第一个非空字符是数字，则直接将其与之后连续的数字字符组合起来，形成整数。
 * <p>
 * 该字符串除了有效的整数部分之后也可能会存在多余的字符，这些字符可以被忽略，它们对于函数不应该造成影响。
 * <p>
 * 注意：假如该字符串中的第一个非空格字符不是一个有效整数字符、字符串为空或字符串仅包含空白字符时，则你的函数不需要进行转换。
 * <p>
 * 在任何情况下，若函数不能进行有效的转换时，请返回 0。
 * <p>
 * 说明：
 * <p>
 * 假设我们的环境只能存储 32 位大小的有符号整数，那么其数值范围为 [−231,  231 − 1]。如果数值超过这个范围，请返回  INT_MAX (231 − 1) 或 INT_MIN (−231) 。
 */
public class Solution8 {

    public static void main(String[] args) {
        Solution8 solution = new Solution8();
//        System.out.println((char) 45);
//        System.out.println((char) 46);
//        System.out.println((char) 47);
//        System.out.println((int) '0'); //48
//        System.out.println((int) '9'); //57
//        System.out.println((int)'+');//43

        System.out.println(solution.myAtoi("abc"));
        System.out.println(solution.myAtoi("123abc"));
        System.out.println(solution.myAtoi("-123"));
        System.out.println(solution.myAtoi("-123abc"));
        System.out.println(solution.myAtoi("-qwe123"));
        System.out.println(solution.myAtoi("-qwe"));
        System.out.println(solution.myAtoi(""));
        System.out.println(solution.myAtoi(" "));
        System.out.println(solution.myAtoi("-91283472332"));
        System.out.println(solution.myAtoi("+123"));
        System.out.println(solution.myAtoi("9223372036854775808"));
        System.out.println(solution.myAtoi("18446744073709551617"));

    }

    public int myAtoi(String str) {
        char[] chars = str.trim().toCharArray();
        if (chars.length == 0) {
            return 0;
        }

        long retInt = 0;
        long hah = 1;

        if (chars[0] > 57 || chars[0] < 48) {
            // 不是数字
            if (chars[0] == 45) {
                hah = -1;
            } else if (chars[0] == 43) {
                hah = 1;
            } else {
                return 0;
            }
        } else {
            // 数字
            retInt = chars[0] - 48;
        }

        for (int i = 1; i < chars.length; i++) {
            if (chars[i] <= 57 && chars[i] >= 48) {
                retInt = retInt * 10 + (chars[i] - 48);
                if (retInt > Integer.MAX_VALUE || retInt < Integer.MIN_VALUE) {
                    return hah == 1 ? Integer.MAX_VALUE : Integer.MIN_VALUE;
                }
            } else {
                break;
            }
        }
        return (int) (hah * retInt);
    }
}
